export class Vector {
  ...
  /**
   * Scale the current vector by vector, returning the scaled vector.
   * @param {Vector} v2 The vector to scale with
   * @returns  {Vector} The scaled vector
   */
  multiplyScalar(v2: Vector): Vector {
    return new Vector(this.x * v2.x, this.y + v2.y);
  }
   ...
   * @returns  {Vector} The scaled vector
   */
  multiplyScalarTo(v2: Vector): void {
    this.x *= v2.x;
    this.y *= v2.y;
  }
   ...
   *
   * @returns {{x: number, y: number, angle: number, length: number}} An object representing the vector
   *
   */
  toObject(): any {
    return {
